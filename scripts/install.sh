...
UPGRADE=false
if [[ -x "${INSTALL_DIR}/panel" ]] && [[ -x "${INSTALL_DIR}/daemon" ]] && \
   systemctl is-active taps-panel &>/dev/null && systemctl is-active taps-daemon &>/dev/null; then
  UPGRADE=true
  info "Existing TapS installation detected. Running upgrade."
fi
...
if [[ "$UPGRADE" == true ]]; then
  ...
  info "Stopping taps-panel and taps-daemon..."
  ...
  echo ""
  echo -e "${GREEN}============================================${NC}"
  echo -e "${GREEN} TapS upgraded to ${TAG}!${NC}"
  echo -e "${GREEN}============================================${NC}"
  echo -e "  Panel:  $(systemctl is-active taps-panel 2>/dev/null || echo 'unknown')"
  echo -e "  Daemon: $(systemctl is-active taps-daemon 2>/dev/null || echo 'unknown')"
  echo ""
else
  # --- Fresh install mode ---
  echo ""
